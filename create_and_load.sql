...
-- At the end it runs 13 exploratory queries on the loaded data.
--
...
-- 7.4 Overall star rating distribution
SELECT overall_rating, COUNT(*) AS hospitals
FROM dim_hospital
GROUP BY overall_rating
ORDER BY overall_rating;

-- 7.5 Spending ratio summary (1.00 = national median)
...
-- 7.6 Spending ratio by star rating
...
-- 7.7 Patient survey stars by spending level
...
-- 7.8 Mortality results worse than national, by ownership
...
-- 7.9 Average infection ratio (SIR) by measure (1.0 = national benchmark)
...
-- 7.10 Spending and star rating by state
SELECT h.state,
...
-- 7.11 Top 10 states by heart failure readmission rate
SELECT h.state,
       COUNT(f.score)         AS hospitals_reporting,
...
-- 7.12 Measurement periods by domain
...
-- 7.13 Spot check one hospital (compare with medicare.gov/care-compare)
