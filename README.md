...
database and then connects to it, and the Query Tool can't switch databases in the
middle of a script.

## Metabase

Metabase runs in Docker. Once it's up (`docker start metabase`) and you've made your
Metabase account, you can connect it with:
...
and creates a few starter questions and a dashboard in a "Group 6: Hospital Quality"
...
- CMS uses "Not Available" and "Not Applicable" inside number columns, which is why we
  load everything as TEXT first and convert afterwards.
...
- Psychiatric and children's hospitals don't get CMS star ratings, so they show up as
  "not rated".
