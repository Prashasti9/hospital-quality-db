...
This guide explains `create_and_load.sql` step by step, including the exploratory queries
at the end (also saved in `exploration.sql`).
...
| File | Purpose |
|---|---|
| `create_and_load.sql` | Builds the database from the beginning, then runs 13 exploratory queries |
| `exploration.sql` | The same 13 queries on their own, to run one at a time in the Query Tool |
...
SELECT ...
...
-- 7.13 Spot check one hospital
SELECT ...
\set ECHO none
```

- The 13 queries are numbered 7.1 to 7.13. They are listed in the table below.
...
## The 13 exploratory queries

These are section 7 of `create_and_load.sql` (numbered 7.1 to 7.13). The same queries
are saved in `exploration.sql`, which contains only SQL, so it also works in the Query
